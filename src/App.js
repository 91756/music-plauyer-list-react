...
class App extends Component {
  state = {musicList: initialTracksList, searchInput: ''}

  deleteMusicItem = id => {
    const {musicList} = this.state
    const updatedList = musicList.filter(eachItem => eachItem.id !== id)
  ...
  /*  onChangeSearchInput = event => {
    ...
    const {musicList} = this.state
    const filteredList = musicList.filter(eachItem =>
  ...
  render() {
    ...
    const showEmptyMusicList = musicList.length === 0
    console.log(showEmptyMusicList)
    return (
      <div>
